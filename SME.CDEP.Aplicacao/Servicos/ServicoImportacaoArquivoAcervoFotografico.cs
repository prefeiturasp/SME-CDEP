            ...
            foreach (var acervoFotograficoLinha in acervosFotograficosLinhas.Where(w=> !w.PossuiErros))
            {
                try
                {
                    var acervoFotografico = new AcervoFotograficoCadastroDTO()
                    {
                        ...
                        CopiaDigital = acervoFotograficoLinha.CopiaDigital.Conteudo.EstaPreenchido() ? acervoFotograficoLinha.CopiaDigital.Conteudo.ToLower().Equals(Constantes.OPCAO_SIM) : false,
                        PermiteUsoImagem = acervoFotograficoLinha.AutorizacaoUsoDeImagem.Conteudo.EstaPreenchido() ? acervoFotograficoLinha.AutorizacaoUsoDeImagem.Conteudo.ToLower().Equals(Constantes.OPCAO_SIM) : false,
                        ConservacaoId = Conservacoes.FirstOrDefault(f => f.Nome.Equals(acervoFotograficoLinha.EstadoConservacao.Conteudo)).Id,
                        ...
                        SuporteId = Suportes.FirstOrDefault(f => f.Nome.Equals(acervoFotograficoLinha.Suporte.Conteudo)).Id,
                        FormatoId = Formatos.FirstOrDefault(f => f.Nome.Equals(acervoFotograficoLinha.FormatoImagem.Conteudo)).Id,
                        TamanhoArquivo = acervoFotograficoLinha.TamanhoArquivo.Conteudo,
                        CromiaId = Cromias.FirstOrDefault(f => f.Nome.Equals(acervoFotograficoLinha.Cromia.Conteudo)).Id,
