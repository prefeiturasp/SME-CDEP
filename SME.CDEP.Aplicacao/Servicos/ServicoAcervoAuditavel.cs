...
namespace SME.CDEP.Aplicacao.Servicos
{
    public class ServicoAcervoAuditavel : IServicoAcervo
    {
        ...
        public async Task<PaginacaoResultadoDTO<AcervoTableRowDTO>> ObterPorFiltro(int? tipoAcervo, string titulo, long? creditoAutorId, string codigo)
        {
            var paginacao = Paginacao;
            try
            {
                ...
                var acervosAgrupandoCreditoAutor = registrosOrdenados
                    ...
                    .Select(async s => new AcervoTableRowDTO
                    {
                        Titulo = s.Key.Titulo,
                        AcervoId = s.Key.Id,
                        Codigo = s.Key.Codigo,
                        ...
                        CapaDocumento = s.Key.TipoAcervoId == (long)TipoAcervo.DocumentacaoHistorica && !string.IsNullOrEmpty(s.Key.CapaDocumento)
                                        ? await ObterImagemBase64(s.Key.CapaDocumento)
                    : null
                    }).Select(t => t.Result);

                var totalRegistros = acervosAgrupandoCreditoAutor.Count();

                var retornoPaginado = new PaginacaoResultadoDTO<AcervoTableRowDTO>()
                {
                    Items = acervosAgrupandoCreditoAutor.Skip(paginacao.QuantidadeRegistrosIgnorados).Take(paginacao.QuantidadeRegistros),
                    TotalRegistros = totalRegistros,
                    TotalPaginas = (int)Math.Ceiling((double)totalRegistros / paginacao.QuantidadeRegistros)
