...
namespace SME.CDEP.Aplicacao.Servicos
{
    public class ServicoImportacaoArquivoAcervoArteGrafica : ServicoImportacaoArquivoBase, IServicoImportacaoArquivoAcervoArteGrafica 
    {
        ...
        public ServicoImportacaoArquivoAcervoArteGrafica(IRepositorioImportacaoArquivo repositorioImportacaoArquivo, IServicoMaterial servicoMaterial,
            ...
            IServicoCromia servicoCromia, IServicoSuporte servicoSuporte,IServicoAcervoArteGrafica servicoAcervoArteGrafica)
            ...
            foreach (var acervoArteGraficaLinha in acervosArtesGraficasLinhas.Where(w=> !w.PossuiErros))
            {
                try
                {
                    var acervoArteGrafica = new AcervoArteGraficaCadastroDTO()
                    {
                        ...
                        CopiaDigital = acervoArteGraficaLinha.CopiaDigital.Conteudo.EstaPreenchido() ? acervoArteGraficaLinha.CopiaDigital.Conteudo.ToLower().Equals(Constantes.OPCAO_SIM) : false,
                        PermiteUsoImagem = acervoArteGraficaLinha.AutorizacaoUsoDeImagem.Conteudo.EstaPreenchido() ? acervoArteGraficaLinha.AutorizacaoUsoDeImagem.Conteudo.ToLower().Equals(Constantes.OPCAO_SIM) : false,
                        ConservacaoId = Conservacoes.FirstOrDefault(f => f.Nome.Equals(acervoArteGraficaLinha.EstadoConservacao.Conteudo)).Id,
                        CromiaId = Cromias.FirstOrDefault(f => f.Nome.Equals(acervoArteGraficaLinha.Cromia.Conteudo)).Id,
                        Largura = double.Parse(acervoArteGraficaLinha.Largura.Conteudo),
                        Altura = double.Parse(acervoArteGraficaLinha.Altura.Conteudo),
                        Diametro = double.Parse(acervoArteGraficaLinha.Diametro.Conteudo),
                        Tecnica = acervoArteGraficaLinha.Tecnica.Conteudo,
                        SuporteId = Suportes.FirstOrDefault(f => f.Nome.Equals(acervoArteGraficaLinha.Suporte.Conteudo)).Id,
                        Quantidade = long.Parse(acervoArteGraficaLinha.Quantidade.Conteudo),
        ...
        public async Task ValidacaoObterOuInserirDominios(IEnumerable<AcervoArteGraficaLinhaDTO> linhas)
        {
            ...
            try
            {
                await ValidarOuInserirCreditoAutoresCoAutores(linhasComsucesso.Select(s => s.Credito.Conteudo).ToArray().UnificarPipe().SplitPipe().Distinct(), TipoCreditoAutoria.Autoria);
