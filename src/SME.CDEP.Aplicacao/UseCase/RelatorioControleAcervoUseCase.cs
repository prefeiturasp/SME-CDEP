...
namespace SME.CDEP.Aplicacao.UseCase
{
    public class RelatorioControleAcervoUseCase : IRelatorioControleAcervoUseCase
    {
        ...
        public RelatorioControleAcervoUseCase(IHttpClientFactory httpClientFactory, IServicoAcervo servicoAcervo, IContextoAplicacao contextoAplicacao)
        {
        ...
        public async Task<Stream> Executar(RelatorioControleAcervoRequest filtros)
        {
            ...
            {
                var resposta = await httpClient.PostAsync("v1/cdep/controle-acervo", new StringContent(mensagem, Encoding.UTF8, "application/json"));
