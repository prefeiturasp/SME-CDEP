...
namespace SME.CDEP.TesteIntegracao
{
    public abstract class BaseMock
    {
        ...
        protected Faker<ImportacaoArquivo> GerarImportacaoArquivo(TipoAcervo tipoAcervo)
        {
            var faker = new Faker<ImportacaoArquivo>("pt_BR");

            faker.RuleFor(x => x.Nome, f=> f.Lorem.Text());
            faker.RuleFor(x => x.TipoAcervo, f=> tipoAcervo);
            faker.RuleFor(x => x.Conteudo, f=> JsonConvert.SerializeObject(GerarAcervoBibliograficoLinhaDTO().Generate(10)));
            faker.RuleFor(x => x.Status, f=> ImportacaoStatus.Pendente);
            AuditoriaFaker(faker);
            return faker;
        }
